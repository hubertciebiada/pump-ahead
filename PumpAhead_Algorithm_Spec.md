...
**Ograniczenia:**
- T_floor_surface ≤ 29°C (EN 1264, heating)
...
- T_floor_surface — opcjonalnie (ochrona 29°C / T_dew i lepsza identyfikacja RC)
...
- T_floor_surface > 29°C → zamknij zawór (hard override, heating)
...
- Safety layer w YAML: T_floor > 29°C → override, fallback na krzywą HP jeśli Python padnie
...
| | Constraint satisfaction: T_floor ≤ 29°C nigdy nie przekroczone |
...
def assert_floor_temp_safe(log):
    """T_floor ≤ 29°C (heating) i T_floor ≥ T_dew + 2°C (cooling)."""
    for r in log:
        assert r.T_floor <= 29.0, f"Floor too hot: {r.T_floor}°C at t={r.t}"
        if r.mode == "cooling":
...
- EN 1264 — max temperatura powierzchni podłogi (29°C strefy przebywania)
