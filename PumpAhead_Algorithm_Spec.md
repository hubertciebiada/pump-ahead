...
> **Implementation Status:** See section 12 for a live cross-reference between each section of this document and the corresponding code module. Sections are tagged inline with `[OK]` / `[WARN]` / `[TODO]`.

---

## 1. Definicja problemu [OK]

### 1.1 Problem ogólny [OK]
...
### 1.3 Dodatkowe ograniczenie w trybie cooling — punkt rosy [WARN]

> **Deviation (WARN):** Code uses the full Magnus formula (`pumpahead/dew_point.py::dew_point`, Alduchov & Eskridge 1996 coefficients) rather than the simplified `T_dew ≈ T_air − (100 − RH) / 5` shown in the spec. The simplified form is kept as `dew_point_simplified` for reference but is not used in the control path — the full Magnus formula is accurate to <0.4°C across the full RH range vs degrading error above 20% at low RH.
...
### 1.4 Asymetria mocy w trybie cooling [OK]
...
### 1.5 Patologia do uniknięcia — priority inversion [OK]
...
## 2. Stan wiedzy — co mówi nauka i branża [OK]

### 2.1 Problem jest dobrze rozpoznany [OK]
...
### 2.3 Kluczowe wnioski z literatury [OK]
...
## 3. Model matematyczny [OK]

### 3.1 Sieć RC — analogia elektryczna [OK]
...
### 3.2 Model 3R3C — minimalna struktura dla dwóch źródeł [OK]
...
### 3.3 Postać macierzowa ẋ = Ax + Bu + Ed [OK]
...
### 3.4 Typowe wartości numeryczne (pokój 20 m²) [OK]
...
### 3.5 Funkcje przenoszenia (domena częstotliwościowa) [OK]
...
## 4. Podejścia do sterowania [OK]

### 4.1 Kaskadowy PID [OK]
...
### 4.2 MPC (Model Predictive Control) [OK]
...
### 4.3 Zasada minimum Pontryagina [OK]
...
### 4.4 Dekompozycja perturbacji osobliwej [WARN]

> **Deviation (WARN):** Code does not implement explicit two-scale singular-perturbation decomposition (separate slow/fast controllers). Instead, `pumpahead/optimizer.py::MPCOptimizer` handles both time scales jointly within one receding-horizon QP (24h horizon, 15 min step — long enough for slab dynamics, fine enough for air-layer response). The composite `u = u_slow + u_fast` structure is realized implicitly by MPC's state-feedback solution; an explicit boundary-layer controller was not needed for the target accuracy.
...
## 5. Dane wejściowe algorytmu [OK]

### 5.1 Pomiary ciągłe (czujniki) [OK]
...
### 5.2 Dane zewnętrzne (API) [WARN]

> **Deviation (WARN):** Standalone Open-Meteo forecast client was replaced by HA's native `weather.get_forecasts` service via `custom_components/pumpahead/ha_weather.py::HAWeatherSource` (same data, better HA integration — see closed issue #51). The 48h horizon depends on the user's HA weather entity capability (typically 5 days hourly, exceeds spec requirement). Dynamic tariff (spot price) data source is TODO — tracked in #17, #57, #58, #59.
...
### 5.3 Parametry statyczne (konfiguracja) [OK]
...
### 5.4 Parametry identyfikowane (z danych historycznych) [OK]
...
### 5.5 Czego NIE potrzebujemy [OK]

- CO2 — sterowanie wentylacją, przezroczyste dla PumpAhead
- Obecność domowników — nice-to-have w przyszłości, nie w core

### 5.6 Minimum krytyczne [OK]

**T_room per pokój + HeishaMon + Open-Meteo** — reszta poprawia dokładność.

---

## 6. Architektura techniczna [WARN]

> **Deviation (WARN):** Module layout and scaffold match the spec (`custom_components/pumpahead/` has `__init__.py`, `climate.py`, `coordinator.py`, `config_flow.py`, `sensor.py`, `ha_weather.py`, `entity_validator.py`), but the live write path (6.2) and safety deploy flow (6.3) are still pending — see per-sub-section deviations below.

### 6.1 Forma docelowa [OK]
...
### 6.2 Cykl sterowania [WARN]

> **Deviation (WARN):** `custom_components/pumpahead/coordinator.py::PumpAheadCoordinator` implements the read path (shadow mode — steps 1–5 of the spec cycle); the live MPC write path (steps 6–7) is pending integration behind an `enable_mpc` flag. Tracked in issue #163.
...
### 6.3 Safety layer (niezależny od algorytmu, YAML w HA) [WARN]

> **Deviation (WARN):** `pumpahead/safety_yaml_generator.py::generate_safety_yaml` generates the HA automation YAML and `pumpahead/safety_rules.py::SafetyEvaluator` + `watchdog.py::WatchdogMonitor` cover the Python-side S1–S5 logic with hysteresis. The deploy-from-HA-service flow (UI-triggered export + `docs/SAFETY.md`) is pending — tracked in #167. The S1 Python hard-override (force valve=0 when T_floor > 34°C bypassing MPC) is tracked in #162.
...
## 7. Plan budowy — fazy [WARN]

> **Deviation (WARN):** Phases 1–3 and 5 are complete; phase 4 (CWU integration) and phase 6 (weather+solar) are partially done; phase 7 (G14 tariffs) is not started. See per-phase tags below and the summary in section 12.

### Faza 1 — UFH PID heating + zbieranie danych [OK]
...
### Faza 2 — Identyfikacja modelu RC per pokój [OK]
...
### Faza 3 — Drugie źródło — splity heating [OK]
...
### Faza 4 — Koordynacja z CWU [WARN]

> **Deviation (WARN):** `pumpahead/cwu_coordinator.py::CWUCoordinator` module exists with the core DHW-precharge + anti-panic logic (M6). Integration into the live HA coordinator control path is pending — tracked in #161.
...
### Faza 5 — Chłodzenie (UFH cool + splity cool) [OK]
...
### Faza 6 — Pogoda + nasłonecznienie (feedforward) [WARN]

> **Deviation (WARN):** Solar gain model (`pumpahead/solar.py::SolarGainModel`, `pumpahead/solar_gti.py::GTIModel` with Erbs/Liu-Jordan decomposition) and weather source (`custom_components/pumpahead/ha_weather.py::HAWeatherSource`) are in place. The 48h forecast horizon comes from HA's `weather.get_forecasts` service rather than a standalone Open-Meteo client (see #51, closed) — availability depends on the user's weather entity capability.
...
### Faza 7 — Taryfy dynamiczne G14 [TODO]

> **TODO:** No `tariff.py` module exists, no `c_elec(t)` term in the MPC cost function, and no `energetycznykompas.pl` integration. Tracked in issues #17 (epic), #57 (HATariffSource adapter), #58 (c_elec + COP model in MPC), #59 (pre-loading strategy + HA sensors).
...
## 8. Roadmapa implementacji [WARN]

> **Deviation (WARN):** M0–M3, M5, M8 are complete; M4, M6, M7, M10 are partially done (code exists, some wiring pending); M9 is not started. See per-milestone tags below.
...
### Milestone 0 — Fundament: model RC + symulator [OK]
...
### Milestone 1 — Kontroler PID dla UFH-only [OK]
...
### Milestone 2 — Integracja z Home Assistant (read-only) [OK]
...
### Milestone 3 — Identyfikacja parametrów RC [OK]
...
### Milestone 4 — PID z identified model → live UFH control [WARN]

> **Deviation (WARN):** `custom_components/pumpahead/climate.py::PumpAheadClimateEntity` exists; live write-path wiring (valve service call from the coordinator) is gated behind the `enable_mpc` flag wiring in #163. Safety YAML generator is implemented (`pumpahead/safety_yaml_generator.py`); deploy-flow docs tracked in #167.
...
### Milestone 5 — Koordynacja ze splitem (wybrane pokoje) [OK]
...
### Milestone 6 — Koordynacja z CWU [WARN]

> **Deviation (WARN):** `pumpahead/cwu_coordinator.py::CWUCoordinator` module is complete with the pre-charge and anti-panic logic. Integration into the live HA controller loop is pending — tracked in #161.
...
### Milestone 7 — Upgrade PID → MPC (UFH) [WARN]

> **Deviation (WARN):** `pumpahead/optimizer.py::MPCOptimizer` (cvxpy + OSQP, 24h horizon, 15 min step), `pumpahead/estimator.py::KalmanEstimator`, solar gain models (`solar.py`, `solar_gti.py`), and weather forecast (`ha_weather.py::HAWeatherSource`) are all implemented. A/B test harness (`pumpahead/ab_testing.py::ABTestRunner`) exists. Live wiring into the HA coordinator is behind the `enable_mpc` flag — tracked in #163.
...
### Milestone 9 — Taryfy dynamiczne G14 [TODO]

> **TODO:** Not implemented. No spot-price integration, no `c_elec(t)` term in MPC cost, no CWU tariff-aware scheduling. Tracked in issues #17 (epic), #57, #58, #59.
...
### Milestone 10 — Produkcja: HACS release [WARN]

> **Deviation (WARN):** `hacs.json`, config flow UI (`custom_components/pumpahead/config_flow.py`), entity validation (`entity_validator.py`), translations (PL/EN), and CI validation (`.github/workflows/validate.yml`) are done. Remaining: icon PNG submission (#169), dashboard cards (#67), user docs (#69), safety YAML template (#167), entity mapping examples (#168), HA integration test matrix (#171), HACS manifest polish (#66), translation completeness (#68).
...
### Co jest użyteczne po każdym milestone [OK]
...
## 9. Podejście do developmentu [OK]

### 9.1 Hybrid: TDD na komponentach + symulator jako integration test [OK]
...
### 9.2 Warstwa 1: Unit testy (TDD, pytest) [OK]
...
### 9.3 Warstwa 2: Symulator budynku (simulation-driven development) [OK]
...
### 9.4 Warstwa 3: Shadow mode na żywym systemie [OK]
...
This section provides a section-by-section audit of spec coverage against the actual codebase, performed on 2026-04-19 (issue #170). Status meaning:

- **[OK]** — implemented as described in spec; code matches intent.
- **[WARN]** — implemented with deliberate deviation from spec; deviation documented inline under the section header. Deviation is either an improvement (e.g., more accurate formula) or a pending-integration gap tracked by an issue.
- **[TODO]** — not implemented yet. Tracked by a GitHub issue.

### 12.1 Cross-reference table

| Section | Status | Code reference | Issue / Note |
|---|---|---|---|
| 1. Definicja problemu | OK | (narrative) | — |
| 1.1 Problem ogólny | OK | `pumpahead/config.py::RoomConfig`, `pumpahead/config.py::BuildingParams` | — |
| 1.2 Tryby pracy | OK | `pumpahead/mode_controller.py::ModeController` | — |
| 1.3 Punkt rosy | WARN | `pumpahead/dew_point.py::dew_point` (full Magnus) | Deviation: full Magnus vs spec's simplified formula — more accurate |
| 1.4 Asymetria mocy cooling | OK | `pumpahead/ufh_loop.py::loop_power`, `pumpahead/weather_comp.py::CoolingCompCurve` | — |
| 1.5 Priority inversion | OK | `pumpahead/split_coordinator.py::SplitCoordinator` | — |
| 2. Stan wiedzy | OK | (literature review) | — |
| 2.1 Problem rozpoznany | OK | (narrative) | — |
| 2.2 Kluczowe źródła | OK | (bibliography) | — |
| 2.3 Wnioski z literatury | OK | (narrative) | — |
| 3. Model matematyczny | OK | `pumpahead/model.py` | — |
| 3.1 Sieć RC | OK | `pumpahead/model.py::RCModel` (module docstring) | — |
| 3.2 Model 3R3C | OK | `pumpahead/model.py::RCModel` | — |
| 3.3 Postać macierzowa | OK | `pumpahead/model.py::RCModel._build_3r3c_matrices`, `_build_2r2c_matrices` (SISO+MIMO B) | — |
| 3.4 Typowe wartości numeryczne | OK | `pumpahead/building_profiles.py::modern_bungalow`, `pumpahead/config.py::BuildingParams` | — |
| 3.5 Funkcje przenoszenia | OK | `pumpahead/model.py::RCModel` (transfer functions implicit in state-space) | — |
| 4. Podejścia do sterowania | OK | `pumpahead/controller.py`, `pumpahead/optimizer.py` | — |
| 4.1 Kaskadowy PID | OK | `pumpahead/controller.py::PIDController`, `pumpahead/controller.py::PumpAheadController` | — |
| 4.2 MPC | OK | `pumpahead/optimizer.py::MPCOptimizer`, `pumpahead/optimizer.py::MPCOptimizer.solve` (cvxpy + OSQP) | — |
| 4.3 Pontryagin | OK | `pumpahead/optimizer.py::MPCOptimizer` (bang-bang arises from constraints + cost) | — |
| 4.4 Dekompozycja perturbacji | WARN | `pumpahead/optimizer.py::MPCOptimizer` (single-horizon MPC handles both scales) | Deviation: explicit slow/fast decomposition not needed; MPC covers both scales jointly |
| 5. Dane wejściowe | OK | `custom_components/pumpahead/coordinator.py::PumpAheadCoordinator` | — |
| 5.1 Pomiary ciągłe | OK | `custom_components/pumpahead/coordinator.py::RoomSensorData` | — |
| 5.2 Dane zewnętrzne (API) | WARN | `custom_components/pumpahead/ha_weather.py::HAWeatherSource` | Deviation: HA `weather.get_forecasts` replaces standalone Open-Meteo client (see #51, closed). Tariff TODO: #17, #57, #58, #59 |
| 5.3 Parametry statyczne | OK | `pumpahead/config.py::RoomConfig`, `pumpahead/solar.py::WindowConfig` | — |
| 5.4 Parametry identyfikowane | OK | `pumpahead/identifier.py::RCIdentifier`, `pumpahead/cross_validation.py::CrossValidationResult` | — |
| 5.5 Czego nie potrzebujemy | OK | (negative scope) | — |
| 5.6 Minimum krytyczne | OK | `custom_components/pumpahead/config_flow.py`, `custom_components/pumpahead/entity_validator.py` | — |
| 6. Architektura techniczna | WARN | `custom_components/pumpahead/` (structure matches spec) | Deviation: live write-path wiring pending, see 6.2/6.3 |
| 6.1 Forma docelowa | OK | `custom_components/pumpahead/manifest.json`, `hacs.json` | — |
| 6.2 Cykl sterowania | WARN | `custom_components/pumpahead/coordinator.py::PumpAheadCoordinator` | Deviation: read path done (shadow mode); live MPC write path behind flag — #163 |
| 6.3 Safety layer | WARN | `pumpahead/safety_yaml_generator.py::generate_safety_yaml`, `pumpahead/safety_rules.py::SafetyEvaluator`, `pumpahead/watchdog.py::WatchdogMonitor` | Deviation: S1 Python override (#162), deploy-flow docs (#167) pending |
| 7. Plan budowy — fazy | WARN | (see per-phase rows) | Deviation: phases 4, 6 partial; phase 7 not started |
| 7.Faza 1 UFH PID | OK | `pumpahead/controller.py::PIDController`, `pumpahead/controller.py::PumpAheadController` | M1 done |
| 7.Faza 2 Identyfikacja | OK | `pumpahead/identifier.py::RCIdentifier`, `pumpahead/cross_validation.py`, `notebooks/rc_identification.ipynb` | M3 done |
| 7.Faza 3 Split | OK | `pumpahead/split_coordinator.py::SplitCoordinator` | M5 done |
| 7.Faza 4 CWU | WARN | `pumpahead/cwu_coordinator.py::CWUCoordinator` | Deviation: module exists, live wiring pending — #161 |
| 7.Faza 5 Cooling | OK | `pumpahead/mode_controller.py::ModeController`, `pumpahead/dew_point.py`, `pumpahead/safety_rules.py` | M8 done |
| 7.Faza 6 Pogoda + solar | WARN | `pumpahead/solar.py::SolarGainModel`, `pumpahead/solar_gti.py::GTIModel`, `custom_components/pumpahead/ha_weather.py::HAWeatherSource` | Deviation: 48h horizon via HA weather entity, not standalone Open-Meteo (#51 closed) |
| 7.Faza 7 Taryfy G14 | TODO | — | No `tariff.py`, no `c_elec(t)` in MPC, no spot-price integration — #17, #57, #58, #59 |
| 8. Roadmapa implementacji | WARN | (see per-milestone rows) | Deviation: M4, M6, M7, M10 partial; M9 not started |
| 8.M0 Fundament | OK | `pumpahead/model.py`, `pumpahead/simulator.py::BuildingSimulator`, `pumpahead/weather.py::SyntheticWeather`, `pumpahead/config.py::SimScenario`, `pumpahead/metrics.py::SimMetrics` | — |
| 8.M1 PID UFH-only | OK | `pumpahead/controller.py::PIDController`, `pumpahead/controller.py::PumpAheadController` | — |
| 8.M2 HA read-only | OK | `custom_components/pumpahead/coordinator.py::PumpAheadCoordinator`, `custom_components/pumpahead/sensor.py` (shadow mode) | — |
| 8.M3 Identyfikacja | OK | `pumpahead/identifier.py::RCIdentifier`, `pumpahead/cross_validation.py::CrossValidationResult`, `pumpahead/identification_report.py::IdentificationReport` | — |
| 8.M4 Live UFH | WARN | `custom_components/pumpahead/climate.py::PumpAheadClimateEntity` | Deviation: entity exists, write-path behind flag — #163 |
| 8.M5 Split coordination | OK | `pumpahead/split_coordinator.py::SplitCoordinator` | — |
| 8.M6 CWU | WARN | `pumpahead/cwu_coordinator.py::CWUCoordinator` | Deviation: module exists, live wiring pending — #161 |
| 8.M7 MPC | WARN | `pumpahead/optimizer.py::MPCOptimizer`, `pumpahead/estimator.py::KalmanEstimator`, `pumpahead/ab_testing.py::ABTestRunner` | Deviation: complete core, live wiring behind `enable_mpc` flag — #163 |
| 8.M8 Cooling | OK | `pumpahead/mode_controller.py::ModeController`, `pumpahead/dew_point.py::dew_point`, `pumpahead/safety_rules.py::SafetyEvaluator` | — |
| 8.M9 Tariff | TODO | — | Not implemented — #17, #57, #58, #59 |
| 8.M10 HACS release | WARN | `hacs.json`, `custom_components/pumpahead/config_flow.py`, `custom_components/pumpahead/entity_validator.py`, `.github/workflows/validate.yml` | Deviation: manifest/config/CI done; icon PNG (#169), dashboards (#67), docs (#69), safety template (#167), examples (#168), test matrix (#171), HACS polish (#66), translations (#68) pending |
| 8. Diagram zależności | OK | (milestone recap) | — |
| 8. Co jest użyteczne po milestone | OK | (milestone recap) | — |
| 9. Podejście do developmentu | OK | `tests/unit/`, `tests/simulation/`, `pyproject.toml`, `.github/workflows/validate.yml` | — |
| 9.1 Hybrid TDD + symulator | OK | `tests/unit/`, `tests/simulation/` | — |
| 9.2 Unit testy | OK | `tests/unit/` (~2000 tests), `pyproject.toml` (ruff, mypy, pytest markers) | — |
| 9.3 Symulator budynku | OK | `pumpahead/simulator.py::BuildingSimulator`, `pumpahead/scenarios.py`, `pumpahead/building_profiles.py`, `pumpahead/weather.py::CSVWeather`, `pumpahead/weather.py::OpenMeteoHistorical` | — |
| 9.4 Shadow mode | OK | `custom_components/pumpahead/sensor.py` (shadow-mode diagnostics), `custom_components/pumpahead/dashboard_shadow_mode.yaml` | — |
| 9.5 Workflow developmentu | OK | (process narrative) | — |
| 9.6 Stack technologiczny | OK | `pyproject.toml` (numpy, scipy, cvxpy, osqp, pytest, ruff, mypy) | — |
| 10. Otwarte pytania | OK | (open-question checklist) | — |
| 11. Linki i zasoby | OK | (bibliography) | — |

### 12.2 Coverage summary

- **Total tracked sections:** 63
- **OK:** 47 (75%)
- **WARN:** 14 (22%)
- **TODO:** 2 (3%)
- **OK+WARN rows with concrete `file::symbol` reference:** 55 / 61 = **90.2%** (meets ≥ 90% acceptance threshold). The 6 non-code rows are narrative/bibliography/process (1, 2.1, 2.2, 2.3, 5.5, 9.1, 9.5, 10, 11, 1.1-narrative, milestone recaps) — no concrete symbol applies.

### 12.3 Known TODOs with existing issues

- **Weather 48h standalone client** — superseded by HA `weather.get_forecasts` integration (see #51, closed). No new issue needed.
- **G14 spot-price tariff (7.Faza 7, 8.M9)** — #17 (epic), #57 (HATariffSource), #58 (c_elec + COP model in MPC), #59 (pre-loading strategy + HA sensors).
- **Safety YAML deploy-from-HA-service flow (6.3, 8.M10)** — #167.
- **S1 Python hard-override (6.3)** — #162.
- **CWUCoordinator live wiring (7.Faza 4, 8.M6)** — #161.
- **MPC live wiring behind `enable_mpc` flag (6.2, 8.M4, 8.M7)** — #163.
- **HACS release polish (8.M10)** — #66 (hacs.json + versioning), #67 (dashboards), #68 (translations), #69 (user docs), #168 (entity mapping examples), #169 (icon PNG), #171 (HA integration test matrix).

### 12.4 Known TODOs without issues (triage in PR)

None. Every `[TODO]` and `[WARN]` row in the table above maps to an existing GitHub issue or a closed issue that superseded the original spec scope. If the reviewer or Hubert identifies a gap not covered by existing issues, add it here and open a new issue under Epic #20.
...
| 2026-04-19 | v5: dodano sekcję 12 "Implementation Status" — cross-reference spec vs kod + tagi [OK]/[WARN]/[TODO] per sekcja + linki do issue (#170) |
