...
<!-- Placeholders pump-ahead, pump-ahead, master, {{ISSUE_NUMBER}} are replaced during installation. -->

# Diamond Workflow — pump-ahead
...
- **NEVER run `git checkout master`** — the base branch is already checked out in the main worktree. Attempting to check it out here will fail with "already used by worktree."
- **Use `git fetch origin master` and reference `origin/master`** whenever you need the latest base branch state. Do not try to switch to it.
- **Your branch is your branch.** Create it, work on it, push it. Never switch away from it to master.
- This applies to ALL agents (planner, dev, reviewer) — none of them should ever attempt to checkout master.

## Entry Point

```
User: claude --worktree pump-ahead-{N}
...
1. **Branch freshness check** (MANDATORY):
   ```bash
   git stash --include-untracked && git fetch origin master && git rebase origin/master && git stash pop && git push --force-with-lease
   ```
...
2. **TL creates PR**:
   ```bash
   gh pr create --base master --title "Issue #{N}: {description}" --body "Closes #{N}"
   ```
...
1. If `git stash --include-untracked && git rebase origin/master` fails with conflicts → state Blocked
...
| Dev pushes without rebase | ALWAYS stash + rebase on master before push |
| Respawning agents endlessly | Max 5 total spawns — then TL takes over or reports BLOCKED |
| Checking out master in a worktree | NEVER checkout master — use `origin/master` as reference |
